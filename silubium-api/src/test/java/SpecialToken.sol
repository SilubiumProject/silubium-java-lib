...
contract SpecialToken is owned, TokenERC20 {
    ...
    event FrozeContract(bool _status);
    ...
    /* Internal transfer, only can be called by this contract */
    function _transfer(address _from, address _to, uint _value) internal {
        require (status);                                       // check contract status ,the contiune for true
    ...
    /// @param _status false can't access any function
    function freezeContract(bool _status) onlyOwner public {
        status = _status;
        emit FrozeContract(_status);
    }
    ...
    function buy() payable public {
        ...
        TokenERC20(tokenAddr).transferFrom(storerAddress, msg.sender, amount);
    ...
    function sell(uint256 amount) public {
        ...
        require(myAddress.balance >= safeMul(amount , sellPrice));   // checks if the contract has enough ether to buy
