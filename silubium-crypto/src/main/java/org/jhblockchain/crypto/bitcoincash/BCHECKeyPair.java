...
import org.jhblockchain.crypto.bitcoin.BitcoinException;
import org.jhblockchain.crypto.bitcoincash.bitcoincash.BitcoinCashAddressFormatter;
import org.jhblockchain.crypto.bitcoincash.bitcoincash.BitcoinCashAddressType;
 ...
 */
@Data
public class BCHECKeyPair extends ECKeyPair {

    private NetworkParameters networkParameters;

    private final boolean testNet;

    public static BCHECKeyPair parse(Key keyPair, boolean testNet, NetworkParameters networkParameters) {
        return new BCHECKeyPair(keyPair, testNet,networkParameters);
    }

    public static BCHECKeyPair parseWIF(String wif,NetworkParameters networkParameters) throws ValidationException {
        ...
        if (decode.length == networkParameters.getRAW_PRIVATE_KEY_COMPRESSED_LENGTH()) {
            isCompressed = true;
        }

        boolean isTestNet = false;
        if ((decode[0] & 0xFF) == networkParameters.getTEST_NET_PRIVATE_KEY_PREFIX()) {
            isTestNet = true;
        }
        ...
        return new BCHECKeyPair(bytes, isTestNet, isCompressed);
    }

    public BCHECKeyPair(byte[] p, boolean testNet, boolean compressed) throws
            ValidationException {
        super(p, compressed);
        this.testNet = testNet;
    }

    public BCHECKeyPair(BigInteger priv, boolean testNet, boolean compressed) {
        super(priv, compressed);
        this.testNet = testNet;
    }

    public BCHECKeyPair(Key keyPair, boolean testNet,NetworkParameters networkParameters) {
        super(keyPair);
        this.testNet = testNet;
        this.networkParameters = networkParameters;
    }

    protected BCHECKeyPair(BigInteger priv, boolean compressed) throws ValidationException {
        super(priv, compressed);
        this.testNet = false;
    }
    ...
    public String getPrivateKey() {
        ...
        byte[] rawPrivateKey = new byte[isCompressed() ? networkParameters.getRAW_PRIVATE_KEY_COMPRESSED_LENGTH() :
                networkParameters.getRAW_PRIVATE_KEY_NO_COMPRESSED_LENGTH()];
        System.arraycopy(bytes, 0, rawPrivateKey, 1, bytes.length);

        rawPrivateKey[0] = (byte) (isTestNet() ? networkParameters.getTEST_NET_PRIVATE_KEY_PREFIX() :
                networkParameters.getMAIN_NET_PRIVATE_KEY_PREFIX());
        if (isCompressed()) {
            rawPrivateKey[rawPrivateKey.length - 5] = (byte)networkParameters.getMAIN_NET_PRIVATE_KEY_SUFFIX();
        }
    ...
    public String getAddress() {
        if(networkParameters instanceof BCHNetworkParameters){
            BCHNetworkParameters bchNetworkParameters = (BCHNetworkParameters)networkParameters;
            return BitcoinCashAddressFormatter.toCashAddress(BitcoinCashAddressType.P2PKH, RIPEMD160.hash160(pubComp),
                    testNet?MoneyNetwork.TEST:MoneyNetwork.MAIN);/*BCHBase32.base32Encode(getRawAddress())*/
        }else {
    ...
    public byte[] getRawAddress() {
        ...
        //拼接测试网络或正式网络前缀
        addressBytes[0] = (byte) (testNet ? networkParameters.getTEST_NET_ADDRESS_SUFFIX() : networkParameters.getMAIN_NET_ADDRESS_SUFFIX());
